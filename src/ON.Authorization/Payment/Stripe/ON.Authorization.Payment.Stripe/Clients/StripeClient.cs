...
namespace ON.Authorization.Payment.Stripe.Clients
{
    public class StripeClient
    {
        ...
        public async Task<StripeNewDetails?> GetNewDetails(
            ...
            ONUser userToken,
            string domainName,
            PaymentModes? paymentMode = PaymentModes.Subscription
        )
        {
            ...
            var url = await CreateCheckoutSession(product, userToken, domainName, paymentMode);
            if (url == null)
                return null;
        ...
        )
        {
            try
            {
                ...
                var chekoutOpts = new global::Stripe.Checkout.SessionCreateOptions
                {
                    ...
                    SuccessUrl =
                        domainName + "/" + paymentMode.ToString().ToLower() + "/stripe/check",
                    CancelUrl = domainName + "/" + paymentMode.ToString().ToLower() + "/",
                    Mode = paymentMode.ToString().ToLower(),
                    LineItems = new()
                    {
